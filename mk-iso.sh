...
#
# UEFI x64 ONLY. No legacy BIOS El Torito entry and no isolinux, by decision --
# shim + MOK enrolment for Secure Boot is a later build. Machines booting this
# ISO must have Secure Boot DISABLED, because the kernel is signed by a key that
# is in no firmware db.
#
...
# --- Preflight ---------------------------------------------------------------
step "Preflight"
for t in xorriso mksquashfs grub-mkstandalone grub-file mkfs.vfat mmd mcopy; do
    command -v "$t" >/dev/null 2>&1 \
        || die "missing host tool: $t
    apt install -y xorriso squashfs-tools grub-efi-amd64-bin dosfstools mtools"
done
...
# The El Torito entry must be a UEFI one, or optical/virtual-CD boot does nothing.
if xorriso -indev "$OUT" -report_el_torito plain 2>/dev/null \
   | grep 'El Torito boot img' | grep UEFI >/dev/null; then
    printf '    OK      El Torito has a UEFI boot image\n'
else
    printf '    FAILED  no UEFI El Torito boot image\n'; fail=$((fail+1))
fi
rm -f "$SAREA"
